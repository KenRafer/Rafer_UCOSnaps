...
if (isset($_POST['createAlbumBtn'])) {
    ...
    if (!empty($album_name)) {
        $createAlbumQuery = createAlbum($pdo, $album_name, $_SESSION['username']);
        $_SESSION['message'] = $createAlbumQuery['message'];
        $_SESSION['status'] = $createAlbumQuery['status'];
        header("Location: ../index.php");
    } else {
...
if (isset($_POST['deleteAlbumBtn'])) {
    ...
    if ($deleteAlbum) {
        ...
        $_SESSION['message'] = "An error occurred while deleting the album.";
        $_SESSION['status'] = '400';
    }
    header("Location: ../index.php");
}
...
if (isset($_POST['updateAlbumBtn'])) {
    ...
    if (!empty($album_name)) {
        $updateAlbum = updateAlbum($pdo, $album_id, $album_name);
